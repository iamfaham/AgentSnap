...
**Adapters** — OpenAI and Anthropic adapters tee `stream=True` calls (`OpenAIRecordingStream` / `AnthropicRecordingStream`): chunks pass through to the caller unmodified while the assembled response is recorded as `raw_response={"__stream__": True, "chunks": [...]}`. Replay rebuilds those chunks into real SDK objects via `replay_stream()`; a streaming recording replayed as a non-streaming request (or vice versa) raises `ReplayError`. Mistral still forces `stream=False` to prevent partial streaming responses from being recorded. Groq and OpenRouter subclass `OpenAIAdapter` directly (OpenAI-compatible interface), so they inherit the tee for free. Not yet supported: the `client.messages.stream()` context-manager helper. All new adapters must either force `stream=False` if the SDK supports streaming, or implement an equivalent recording tee. A stream that is never iterated and never closed is finalized automatically at recorder/asserter exit (`TraceAccumulator.finalize_streams()`), but consuming or closing it promptly is still recommended so events appear in call order.

**Async client interception** (`patches.py`: `_apply_anthropic_async`, `_apply_openai_async`) — `PatchSet` patches `AsyncMessages.create` / `AsyncCompletions.create` the same way it patches the sync classes: replay branch, then stream branch, then non-stream, mirrored 1:1 with `async def`/`await`. Async stream tees (`AsyncAnthropicRecordingStream` / `AsyncOpenAIRecordingStream` in `adapters/anthropic.py` / `adapters/openai.py`) use `__aiter__`/`aclose()`/`__aenter__`/`__aexit__`, but expose a **sync** `close()` that records without touching the inner stream — this lets `TraceAccumulator.finalize_streams()` guarantee an event for an abandoned stream without needing an event loop at teardown. `replay_stream_async()` per adapter module mirrors the sync replay stream for async consumers.

**OpenAI Responses API** (`patches.py`: `_apply_openai_responses`, `_apply_openai_responses_async`; helpers in `adapters/openai.py`: `extract_responses_text`, `extract_responses_tool_requests`, `reconstruct_response`) — non-streaming `responses.create()`/`AsyncResponses.create()` calls are intercepted and replayed the same way as chat completions. `kwargs["input"]` is normalized to a messages list (str -> single user message; `instructions` prepended as a system message). Streamed Responses-API calls (`stream=True`) pass through **unrecorded** — the one remaining capture hole, documented in README/USAGE.

**Framework compatibility** — frameworks (Pydantic AI, OpenAI Agents SDK, LangChain) build SDK clients internally, so they work through `PatchSet` alone with no per-framework wrapper code. Real-framework verification tests live in `tests/frameworks/` (`pytest.importorskip`-guarded, `pytest.mark.frameworks`), installed via the `frameworks` extra and exercised by a separate `frameworks` CI job (`.[dev,frameworks]`) so the main test matrix stays hermetic (no network, no framework deps required).
