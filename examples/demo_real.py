...
Demonstrates three things:
  1. Catching unintended regressions (structural + argument drift)
  2. Intentional model upgrade with approval
  3. CI simulation via pytest

Set whichever API keys you have in .env -- providers without a key are skipped.
OpenRouter is used for the three scenario demos (only key needed for those).

Required env vars:
    OPENROUTER_API_KEY   -- used for scenarios 1, 2, 3
...
def scenario_1_unintended_change(key: str) -> None:
    ...
    try:
        ...
        print(f"  Structural    : {r.structural_diff}")
        print()
